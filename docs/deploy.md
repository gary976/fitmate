...
3. 展开 **高级配置**：
   - 环境变量：添加 `ARK_API_KEY` = 方舟 **Claude Code 套餐的 Key**（`ark-` 开头）。函数会自动把 OpenAI 格式翻译成套餐通道（`/api/plan`）的 Anthropic 格式，复用套餐额度，**无需单独开通方舟按量付费**。Key 存云端，App 里留空更安全。
   - 启动命令（如要求填写）：`node index.js`
...
| 模型名称 | 不用改（函数会固定走套餐的 glm-5.3-flash） |
...
- **AI 报 401**：函数环境变量里的 Key 不是 Claude Code 套餐 Key（必须是方舟套餐/plan 通道可用的 Key），或复制时缺字符/多空格。
